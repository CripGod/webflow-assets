...
const SU='https://script.google.com/macros/s/AKfycbwfxkkmMah2DgJHH9LmbwWWXCIggQSz8zWxRn9UwyEcemorjr_sKJ4iGPjBCk2k5FI3/exec';
...
async function fetchSheets(){
  const [r1,r2]=await Promise.all([
    fetch('https://sheets.googleapis.com/v4/spreadsheets/'+SID+'/values/Sheet1!A1:U200?key='+HK),
...
async function syncData(){
  if(document.activeElement?.classList.contains('due-input'))return;
...
function parseJobs(rows){
  const grouped={},order=[],archGroups={};
  rows.forEach(row=>{
    const g=(row[C.D]||'').toString().trim().toLowerCase();
    if(!g||g==='group tag'||g==='group')return;
    if(!grouped[g]){grouped[g]=[];order.push(g);}
    grouped[g].push(row);
    if((row[20]||'').toString().trim().toUpperCase()==='TRUE')archGroups[g]=true;
  });
  allJobs=order.filter(g=>g!=='pending'&&!archGroups[g]).map(k=>bJ(k,grouped[k]));
  archivedItems=Object.keys(archGroups).filter(g=>grouped[g]).map(k=>bJ(k,grouped[k]));
...
function editDue(e){
  ...
  if(cur){const m=String(cur).match(/^(\d{4}-\d{2}-\d{2})/);inp.value=m?m[1]:'';}
  btn.replaceWith(inp);inp.focus();inp.showPicker?.();
  let done=false;
  const commit=()=>{
    if(done||!document.body.contains(inp))return;done=true;
    const v=inp.value;
    if(v!==cur){
      const job=allJobs.find(j=>j.key===key);if(job)job.delivery=v||null;
      if(v)fetch(SU,{method:'POST',body:JSON.stringify({action:'job',isEdit:true,editGroup:key,delivery:v})}).catch(()=>{});
...
function pDate(s){if(!s)return null;const m=String(s).match(/^(\d{4})-(\d{2})-(\d{2})/);return m?new Date(+m[1],+m[2]-1,+m[3]):new Date(s);}
function dDue(due,id){
  if(!due)return '<button class="dd-btn" data-id="'+id+'" onclick="editDDue(event)">+ date</button>';
  const dt=pDate(due),today=new Date();today.setHours(0,0,0,0);const ov=dt<today;
...
function editDDue(e){
  ...
  const commit=()=>{
    ...
    if(v!==cur){
      ...
      fetch(SU,{method:'POST',body:JSON.stringify({action:'delivDue',id:id,due:v})}).catch(()=>{});
      setTimeout(syncData,5000);
    }
...
function confirmDelete(){if(!deleteTarget)return;const k=deleteTarget;fetch(SU,{method:'POST',body:JSON.stringify({action:'archive',group:k,archived:true})}).catch(()=>{});closeModal();setTimeout(connect,800);}
function rJ(key){fetch(SU,{method:'POST',body:JSON.stringify({action:'archive',group:key,archived:false})}).catch(()=>{});setTimeout(connect,800);}
