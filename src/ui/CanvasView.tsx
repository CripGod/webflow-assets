...
export function CanvasView() {
  const { cfg, update, zoom, setZoom, panMode, setPanMode, gridStyle, setGridStyle, phase, setPhase, selectedState, setSelectedState, canvasMode, setCanvasMode, board, library, moveBoardItem, scaleBoardItem, removeBoardItem, bgImage, setBgImage, focus, setFocus, kitShapes, kitSizes, kitTextOy, bgShow, bgOpacity, bgBlur } = useGen();
  ...
  const heroSvg = useMemo(
    () => (focus ? renderKit(cfg, focus, fSize, displayed, focus === "toggle" && displayed === "pressed" ? 0 : undefined, kitShapes[focus], { textOy: fOy }) : renderBevel(cfg, displayed)),
    [cfg, displayed, focus, kitShapes, fSize, fOy]
  );
        ...
        <div className="stack" aria-label="State previews">
          {(focus === "toggle" || focus === "badge"
            ...
            <button className={`scard clickable${s === selectedState ? " sel" : ""}`} key={s}
              ...
              <div className="scard-body" dangerouslySetInnerHTML={{ __html: focus ? renderKit(cfg, focus, fSize, s, v, kitShapes[focus], { textOy: fOy }) : renderBevel(cfg, s) }} />
            </button>
          ))}
        </div>
      )}
